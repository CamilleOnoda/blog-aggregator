...
Never miss a post from your favorite blogs. `blog-aggregator` is a command-line tool that lets you subscribe to RSS feeds and automatically pulls new posts into a local database, so you can browse everything in one place, from the terminal, without opening a browser.
...
```
blog-aggregator/
├── main.go              # CLI commands, RSS fetching logic, feed scraper
├── internal/
│   ├── config/          # Config file handling, RSS types
│   └── database/        # Type-safe database layer (generated by sqlc)
├── sql/
